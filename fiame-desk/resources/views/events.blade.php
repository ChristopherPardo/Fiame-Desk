<x-app-layout>
    @if(Auth::user()->id == 1)
    <div x-data="{fGathering : {}, open : false, confirmed : false, showConfirmation : false,  gatherings : @js($gatherings->pluck('date'))}">
        <div class="relative">
            <button class="absolute z-10 right-0 top-0 h-16 w-16 m-5" x-on:click="open = ! open">
                <svg x-show="! open" xmlns="http://www.w3.org/2000/svg" class="h-13 w-13" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="w-full max-w-xs m-5" x-show="open">
            <form method="POST" action="{{ route('events.store') }}" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" x-ref="form">
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name='description' id="description" type="text" :value="fGathering.description ?? '{{old('description')}}'" placeholder="Description">
                    @error('description')
                ...
                <div class="flex items-center justify-center">
                    <div class="datepicker relative form-floating mb-3 xl:w-96" data-mdb-toggle-button="false">
                        ...
                        <input name="date" type="date" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" :value="fGathering.date ?? '{{old('date')}}'" x-ref="date" placeholder="20/11/2019" />
